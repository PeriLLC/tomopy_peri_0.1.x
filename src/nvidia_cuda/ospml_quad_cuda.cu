...
void ospml_quad_forEachSlice( CCudaData *pData, int s)
{
	float mov;
...
void ospml_quad_forEachIter( CCudaData *pData)
{
	pData->cleanSimdata();

	for (int s=0; s<pData->host_dy; s++)
		ospml_quad_forEachSlice(pData ,s);
}
