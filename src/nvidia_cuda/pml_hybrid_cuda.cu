...
void pml_hybrid_forEachSlice( CCudaData *pData, int s)
{
	float mov;
...
void pml_hybrid_forEachIter( CCudaData *pData)
{
	pData->cleanSimdata();

	for (int s=0; s<pData->host_dy; s++)
		pml_hybrid_forEachSlice(pData ,s);
}
