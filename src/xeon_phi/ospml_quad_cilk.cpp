...
void ospml_quad_forEachProjection(const int dx, const int dy, const int dz,
...
void ospml_quad_forEachSlice(const int dx, const int dy, const int dz,
		const int ngridx, const int ngridy, int s, float* center,
		float * sinp, float* cosp, int* quadrant, float * data, float *reg_pars,
		float * recon, float * simdata,int num_block, float *ind_block)
{
	float mov;
	float *E, *F, *G;
    ...
    for (int os=0; os<num_block+1; os++)
    {
        if (os == num_block)
        {
	...
	//	float sum_dist2;
		...
		sum_dist = (float*)_mm_malloc((ngridx*ngridy)*sizeof(float), 64);
		//E = (float *)calloc((ngridx*ngridy), sizeof(float));
		E = (float*)_mm_malloc((ngridx*ngridy)*sizeof(float), 64);
		//F = (float *)calloc((ngridx*ngridy), sizeof(float));
		F = (float*)_mm_malloc((ngridx*ngridy)*sizeof(float), 64);
		//G = (float *)calloc((ngridx*ngridy), sizeof(float));
		G = (float*)_mm_malloc((ngridx*ngridy)*sizeof(float), 64);
	...
	//    ANNOTATE_SITE_BEGIN( EachProjection );  // Place before the loop control statement to begin a parallel code region (parallel site).
		...
		{
			int p = ind_block[q+os*subset_ind1];
//            std::cout <<os<<","<<q<<","<<p<<","<<sinp[p]<<std::endl;
	//       	ANNOTATE_ITERATION_TASK( MyTask1 );  // Place at the start of loop body. This annotation identifies an entire body as a task.
			ospml_quad_forEachProjection(dx, dy, dz, ngridx, ngridy,s,p,sinp[p],cosp[p],quadrant[p],mov,gridx,gridy,
...
void ospml_quad_forEachIter(int dx, int dy, int dz, int ngridx, int ngridy, float* center,
		float* sinp, float* cosp, int* quadrant, float * data, float *reg_pars, float* recon
		, float *simdata,int num_block, float *ind_block)
{

	simdata[0:(dx*dy*dz)]=0.0;

//	for (int kk=0;kk<num_block;kk++)
//		std::cout << "kk"<<kk<<","<<ind_block[kk]<<std::endl;
...
//        ANNOTATE_SITE_BEGIN( EachSlice );  // Place before the loop control statement to begin a parallel code region (parallel site).
    ...
    {
	//        	ANNOTATE_ITERATION_TASK( MyTask1 );  // Place at the start of loop body. This annotation identifies an entire body as a task.
   		ospml_quad_forEachSlice(dx, dy, dz, ngridx, ngridy, s, center,
...
//    	recon[0:dy*ngridx*ngridy]=1e-6;
        ...
        for (int i=0; i<num_iter; i++)
        	ospml_quad_forEachIter(dx, dy, dz, ngridx, ngridy, center,
        			sinp,cosp,quadrant, data,reg_pars, recon,  simdata,
					num_block, ind_block);
        _mm_free(simdata);
    }
