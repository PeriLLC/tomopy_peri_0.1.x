...
void ospml_hybrid_forEachProjection(const int dx, const int dy, const int dz,
...
void ospml_hybrid_forEachSlice(const int dx, const int dy, const int dz,
...
//            std::cout <<os<<","<<q<<","<<p<<","<<sinp[p]<<std::endl;
	//       	ANNOTATE_ITERATION_TASK( MyTask1 );  // Place at the start of loop body. This annotation identifies an entire body as a task.
			ospml_hybrid_forEachProjection(dx, dy, dz, ngridx, ngridy,s,p,sinp[p],cosp[p],quadrant[p],mov,gridx,gridy,
...
void ospml_hybrid_forEachIter(int dx, int dy, int dz, int ngridx, int ngridy, float* center,
...
//        ANNOTATE_SITE_BEGIN( EachSlice );  // Place before the loop control statement to begin a parallel code region (parallel site).
    ...
    {
	//        	ANNOTATE_ITERATION_TASK( MyTask1 );  // Place at the start of loop body. This annotation identifies an entire body as a task.
   		ospml_hybrid_forEachSlice(dx, dy, dz, ngridx, ngridy, s, center,
...
//    	recon[0:dy*ngridx*ngridy]=1e-6;
        ...
        for (int i=0; i<num_iter; i++)
        	ospml_hybrid_forEachIter(dx, dy, dz, ngridx, ngridy, center,
        			sinp,cosp,quadrant, data,reg_pars, recon,  simdata,
					num_block, ind_block);
        _mm_free(simdata);
    }
